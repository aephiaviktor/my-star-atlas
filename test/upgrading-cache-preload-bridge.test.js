...
function loadBridge(clock = { now: 0 }) {
  ...
  return { bridge: exposed.upgradingCache, exposeCalls, source };
}
...
test('preload imports frozen modules and extends the existing namespace once with a fixed descriptor', () => {
  ...
  assert.equal(bridge.buildKey(input()), 'msa:earnings-cache:{"datasetScope":"upgrading-ledger","faction":"MUD","filters":{},"playerProfile":"ProfileA","schemaVersion":"1","section":"earnings","subtab":"upgrading"}');
});
...
test('bridged ensure is single-flight, clone-safe, force-aware, and identity-isolated', async () => {
  ...
  pending.resolve({ ok: true, upgradingRows: [{ asset: 'A' }] });
  ...
  await Promise.resolve(); assert.equal(forcedCalls,1); forced.resolve({ok:true,upgradingRows:[{asset:'B'}]}); await Promise.all([c,d]);
});

test('stale last-known-good is inspectable before revalidation completes', async () => {
  const clock={now:0}; const {bridge}=loadBridge(clock);
  await bridge.ensure(input(),()=>Promise.resolve({ok:true,upgradingRows:[1]}));
  clock.now=stateModule.UPGRADING_CACHE_FRESHNESS_MS;
  const stale=bridge.inspect(input()); assert.equal(stale.entry.status,'stale'); assert.deepEqual(stale.entry.value,{ok:true,upgradingRows:[1]});
  const wait=deferred(); const revalidating=bridge.ensure(input(),()=>wait.promise);
  assert.deepEqual(bridge.inspect(input()).entry.lastGoodValue,{ok:true,upgradingRows:[1]}); wait.resolve({ok:true,upgradingRows:[2]}); await revalidating;
});
