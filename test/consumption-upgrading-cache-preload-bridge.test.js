...
function loadBridge(clock = { now: 0 }) {
  ...
  return { bridge: exposed.consumptionUpgradingCache, exposeCalls, source };
}
const input = (faction='MUD', playerProfile='ProfileA', force=false, componentFilter='', starbaseFilter='') => ({ faction, playerProfile, componentFilter, starbaseFilter, force });
...
test('preload imports frozen modules and extends the existing namespace once with a fixed descriptor', () => {
  ...
  assert.equal(bridge.buildKey(input()), 'msa:earnings-cache:{"datasetScope":"upgrade-consumption-31d","faction":"MUD","filters":{"componentFilter":"","starbaseFilter":""},"playerProfile":"ProfileA","schemaVersion":"1","section":"consumption","subtab":"upgrading"}');
});
...
test('bridged ensure is single-flight, clone-safe, force-aware, and identity-isolated', async () => {
  ...
  assert.notEqual(bridge.buildKey(input('MUD','ProfileA',false,'A','')), ra.key); assert.notEqual(bridge.buildKey(input('MUD','ProfileA',false,'','S')), ra.key);
...
test('delayed old-profile and old-filter completions cannot populate the active canonical entry', async () => {
  ...
  const filterOld=input('MUD','ProfileB',false,'OLD','SB1');
  const filterNew=input('MUD','ProfileB',false,'NEW','SB2');
...
test('stale last-known-good is inspectable before revalidation completes', async () => {
  ...
  clock.now=stateModule.CONSUMPTION_UPGRADING_CACHE_FRESHNESS_MS;
