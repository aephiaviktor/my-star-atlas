...
const { BREAKEVEN_CACHE_FRESHNESS_MS, UPGRADING_CACHE_FRESHNESS_MS, CONSUMPTION_UPGRADING_CACHE_FRESHNESS_MS, CONSUMPTION_SCANNING_CACHE_FRESHNESS_MS, CONSUMPTION_MINING_CACHE_FRESHNESS_MS, CONSUMPTION_CARGO_CACHE_FRESHNESS_MS, CONSUMPTION_CRAFTING_CACHE_FRESHNESS_MS, CONSUMPTION_TOTAL_CACHE_FRESHNESS_MS, createEarningsCacheState } = require('../electron/earnings-cache-state');
...
test('freshness constant and exact boundary use successful completion time', async () => {
  ...
  assert.equal(CONSUMPTION_CARGO_CACHE_FRESHNESS_MS, 900000);
  assert.equal(CONSUMPTION_CRAFTING_CACHE_FRESHNESS_MS, 900000);
  assert.equal(CONSUMPTION_TOTAL_CACHE_FRESHNESS_MS, 900000);
  let now = 1000;
