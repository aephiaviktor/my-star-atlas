...
function loadBridge(clock = { now: 0 }) {
  ...
  return { bridge: exposed.consumptionTotalCache, exposeCalls, source };
}
const input = (faction='MUD', playerProfile='ProfileA', force=false, starbaseFilter='', assetFilter='') => ({ faction, playerProfile, starbaseFilter, assetFilter, force });
...
test('preload imports frozen modules and extends the existing namespace once with a fixed descriptor', () => {
  ...
  assert.equal(bridge.buildKey(input()), 'msa:earnings-cache:{"datasetScope":"total-consumption-31d","faction":"MUD","filters":{"assetFilter":"","starbaseFilter":""},"playerProfile":"ProfileA","schemaVersion":"1","section":"consumption","subtab":"total"}');
});
...
test('stale last-known-good is inspectable before revalidation completes', async () => {
  ...
  clock.now=stateModule.CONSUMPTION_TOTAL_CACHE_FRESHNESS_MS;
