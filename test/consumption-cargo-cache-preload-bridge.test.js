...
function loadBridge(clock = { now: 0 }) {
  ...
  return { bridge: exposed.consumptionCargoCache, exposeCalls, source };
}
...
test('preload imports frozen modules and extends the existing namespace once with a fixed descriptor', () => {
  ...
  assert.equal(bridge.buildKey(input()), 'msa:earnings-cache:{"datasetScope":"cargo-consumption-31d","faction":"MUD","filters":{"fleetFilter":"","starbaseFilter":""},"playerProfile":"ProfileA","schemaVersion":"1","section":"consumption","subtab":"cargo"}');
});
