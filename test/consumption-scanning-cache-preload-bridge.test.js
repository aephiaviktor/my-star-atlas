...
function loadBridge(clock = { now: 0 }) {
  ...
  return { bridge: exposed.consumptionScanningCache, exposeCalls, source };
}
const input = (faction='MUD', playerProfile='ProfileA', force=false, starbaseFilter='', fleetFilter='') => ({ faction, playerProfile, starbaseFilter, fleetFilter, force });
...
test('preload imports frozen modules and extends the existing namespace once with a fixed descriptor', () => {
  ...
  assert.equal(bridge.buildKey(input()), 'msa:earnings-cache:{"datasetScope":"scan-consumption-31d","faction":"MUD","filters":{"fleetFilter":"","starbaseFilter":""},"playerProfile":"ProfileA","schemaVersion":"1","section":"consumption","subtab":"scanning"}');
});
...
test('stale last-known-good is inspectable before revalidation completes', async () => {
  ...
  clock.now=stateModule.CONSUMPTION_SCANNING_CACHE_FRESHNESS_MS;
