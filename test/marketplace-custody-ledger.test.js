...
test('withdrawal basis follows uncosted-first inventory and crosses wallets into a faction deposit', () => {
  const events = [
  ...
  assert.equal(row.quantity, 10);
  assert.equal(row.costedQuantity, 0);
  assert.equal(row.uncostedQuantity, 10);
  assert.equal(row.carriedBasisAtlas, 6);
  assert.equal(row.finalBasisAtlas, 10);
  assert.equal(row.costPerUnitAtlas, 1);
  assert.equal(row.status, 'Unvalued');
});

test('successive withdrawals deplete one observed source pool in uncosted-first order', () => {
  const observations = [{
    faction: 'MUD', starbase: 'MUD-1', asset: 'Fuel', timestamp: '2026-08-31T10:00:00Z',
    quantity: 100, knownQuantity: 80, uncostedQuantity: 20, weightedAveragePriceAtlas: 2,
  }];
  ...
  assert.equal(first.costedQuantity, 0);
  assert.equal(first.uncostedQuantity, 15);
  assert.equal(second.costedQuantity, 5);
  assert.equal(second.uncostedQuantity, 5);
  assert.equal(second.carriedBasisAtlas, 10);
});
...
test('deposit without a matching custody lot is Review rather than silently valued', () => {
