...
function loadBridge(clock = { now: 0 }) {
  ...
  return { bridge: exposed.consumptionMiningCache, exposeCalls, source };
}
...
test('preload imports frozen modules and extends the existing namespace once with a fixed descriptor', () => {
  ...
  assert.equal(bridge.buildKey(input()), 'msa:earnings-cache:{"datasetScope":"mining-consumption-31d","faction":"MUD","filters":{"fleetFilter":"","starbaseFilter":""},"playerProfile":"ProfileA","schemaVersion":"1","section":"consumption","subtab":"mining"}');
});
...
test('stale last-known-good is inspectable before revalidation completes', async () => {
  ...
  clock.now=stateModule.CONSUMPTION_MINING_CACHE_FRESHNESS_MS;
