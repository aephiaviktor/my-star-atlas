...
test('corrupt or mismatched checkpoint fails safely without loading basis', async () => {
  ...
  try {
    ...
    await fs.writeFile(filePath, JSON.stringify({ schemaVersion: 5, faction: 'MUD', profile: 'MUD', ledgerRows: [], seenEventFingerprints: [] }));
...
test('legacy checkpoint is invalidated so corrected provenance is replayed', async () => {
  ...
  try {
    await fs.writeFile(filePath, JSON.stringify({ schemaVersion: 2, faction: 'ONI', profile: 'USTUR', ledgerRows: [], seenEventFingerprints: [] }));
