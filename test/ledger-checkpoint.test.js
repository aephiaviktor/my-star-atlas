...
test('corrupt or mismatched checkpoint fails safely without loading basis', async () => {
  ...
  try {
    ...
    await fs.writeFile(filePath, JSON.stringify({ schemaVersion: LEDGER_CHECKPOINT_SCHEMA_VERSION, faction: 'MUD', profile: 'MUD', ledgerRows: [], seenEventFingerprints: [] }));
...
test('v0.6.301 checkpoint is invalidated so acquisition cargo attribution is rebuilt', async () => {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), 'msa-ledger-checkpoint-'));
  const filePath = path.join(directory, 'ONI.json');
  try {
    await fs.writeFile(filePath, JSON.stringify({ schemaVersion: 16, faction: 'ONI', profile: 'USTUR', ledgerRows: [], seenEventFingerprints: [] }));
