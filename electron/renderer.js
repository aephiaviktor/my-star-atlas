...
function renderUpgradingChartAxes(svg, { minY = 0, maxY = 1, xMax = 24, xTicks = [0,6,12,18,24], xLabel = '', yLabel = '' } = {}) {
  const width=760,height=280,left=74,right=16,top=12,bottom=48;
...
function renderUpgradingOptimizationAnalytics() {
  ...
  if(svg && analytics.scatter.length){ const maxX=Math.max(1,...analytics.scatter.map(r=>r.factionLp))*1.08,maxY=Math.max(1,...analytics.scatter.map(r=>r.playerLpPerCrew))*1.08; const axes=renderUpgradingChartAxes(svg,{maxY,xMax:maxX,xTicks:[0,maxX/4,maxX/2,maxX*3/4,maxX],xLabel:'Faction LP redeemed',yLabel:'Player LP / avg phantom crew'});
