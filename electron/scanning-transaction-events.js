...
function aggregateScanningTransactionEvents(rows, { includedDays = null } = {}) {
  ...
  for (const row of rows || []) {
    ...
    if (!['sdu', 'movement'].includes(measurement)) continue;
    if (measurement === 'movement' && assignment !== 'Scan') continue;
    ...
    if (!fleet || !timestamp || !isoDate || !Number.isFinite(value) || value < 0) continue;
...
module.exports = { aggregateScanningTransactionEvents };
